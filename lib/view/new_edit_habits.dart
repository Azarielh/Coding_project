...
class NewEditHabit extends StatelessWidget {
  ...
  const NewEditHabit({Key? key, this.habit}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    AllFieldsFormBloc bloc = AllFieldsFormBloc();

    if (habit != null) {
      if (habit!.interval != null) {
        double intervalValue =
            Habit.availableIntervals.indexOf(habit!.interval!).toDouble() / 2;
        bloc.interval.changeValue(intervalValue);
      }
      double frequenceValue =
          Habit.availableFrequences.indexOf(habit!.frequence).toDouble() / 2;
      bloc.frequence.changeValue(frequenceValue);

      bloc.iteration.changeValue(habit!.iteration.toString());
      bloc.designation.changeValue(habit!.designation);
      if (habit!.jours != null) {
        bloc.jours.changeValue(habit!.jours!.split(" "));
      }
    }
